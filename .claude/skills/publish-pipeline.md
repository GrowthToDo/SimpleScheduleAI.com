...
7. **Cold read (added 2026-08-27)**: a SEPARATE pass whose only job is to fail to understand things. Every other phase asks "is this correct?"; this one asks "could a nurse manager act on it?" Added because what-is-a-baylor-shift passed the mechanical gate, the proofread agent and the fact-check agent with zero failures, then took three rounds of founder feedback that were ALL comprehension: shorthand never defined, headings asking about an unnamed "it", a legal section written in the statute's vocabulary, no worked numbers.
   ...
   Fix everything it names, then re-run the mechanical gate. Findings fold into `proofread`; the gate verdict still lives there.
   **Also have it emit a JSON observations block** (schema in `docs/seo/content-scoring.md`) and record it: `npm run cold-read record <slug> --from <file.json>`. The agent reports only OBSERVATIONS (which sentences it reread, which terms went undefined, which headings failed to answer themselves, where it quit, what it would do Monday vs what the post asked for without equipping it). The script does the arithmetic, because an agent asked for a score invents one. Set `intent` to `act`, `trust` or `both` depending on what the post is for. **ADVISORY until ~7 posts are scored and the constants are calibrated** (founder 2026-09-21); it is advisory by construction, since scores live in `manifest.scores` which the gate never reads. `npm run cold-read trend` shows the countdown and whether we are improving. **Never self-run this straight after drafting** — the author cannot fail to understand their own shorthand, which is exactly why the themes pass missed all of it.
