...
## BLOCKED DRAFTS (check before starting any publish)

These drafts cannot be published by running the normal phases. Their SSAI
differentiator rests on a claim we do not have, so a find/replace leaves them
arguing for a capability that does not exist. Each needs its differentiator
rewritten first.
...
filtering / enforcement / expiry in our own voice.
