...
5. Write to `./.tmp/handoff/context-<timestamp>.md`. Copy as latest (Windows — symlinks need admin):

   ```powershell
   Copy-Item ".\.tmp\handoff\context-<timestamp>.md" ".\.tmp\handoff\context-latest.md" -Force
   ```

6. Estimate compression. `words * 1.3` ≈ tokens. Report ratio.

7. Copy to clipboard (Windows):

   ```powershell
   Set-Clipboard -Value "/handoff resume"
   ```
...
1. Read `./.tmp/handoff/context-latest.md` in full. If missing, find most recent:

   ```powershell
   Get-ChildItem .tmp\handoff\context-*.md | Where-Object { $_.Name -ne "context-latest.md" } | Sort-Object LastWriteTime -Descending | Select-Object -First 1
   ```
