...
- **Editing or creating a skill:** Load `.claude/skills/skill-authoring.md` before touching anything in `.claude/skills/`. It carries this repo's conventions: the router plus reference-file split, the test for what belongs in a skill versus a doc under `docs/`, and the insert-inside-the-existing-structure rule that two real regressions came from breaking.
