...
excerpt: >
  A scheduling app that logs you out halfway through a schedule is usually doing what
  it was set up to do. The idle timer is a security setting, someone at the vendor or
  your hospital chose it, and it can often be changed.
...
- Nurses notice login friction in reviews. One ShiftWizard reviewer wished they did not "have to input my info every time I log in."
...
There are other causes too. A browser setting or extension that clears cookies can end a session early. Logging in on a second device can sign you out on the first. A mobile app that has not been updated for a new phone operating system can loop back to the login screen.
...
Ask two people. Ask the vendor what the idle timeout is on your account and whether it can be changed. Then ask your IT contact whether the hospital sets a timeout that applies to the scheduling app. Either answer tells you who can change it. The table shows each cause and who controls it.
...
Login friction shows up in public reviews, though the complaints are about logging in often, not about timers by name. A licensed vocational nurse reviewed ShiftWizard on Capterra. They wrote: "I wish the messaging was better, and that I have to input my info every time I log in." The same reviewer liked that "I know my schedule at a glance, it is clear and direct."

A second case is about where you have to log in, not how often. A chief nursing officer reviewed NurseGrid on Capterra and wrote: "It no longer has the manager app so I have to login to desktop to make changes." For a manager who edits the schedule from a phone between rounds, losing the mobile app means waiting until they reach a desk.
...
Then work around the timer. Save each time you finish a block of shifts. Before you leave the screen for more than a few minutes, save first.
