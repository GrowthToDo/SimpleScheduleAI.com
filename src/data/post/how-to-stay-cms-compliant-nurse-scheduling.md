...
**On-duty nurse, [§485.631(a)(5)](https://www.ecfr.gov/current/title-42/chapter-IV/subchapter-G/part-485/subpart-F/section-485.631) [2]:** A registered nurse, clinical nurse specialist, or licensed practical nurse must be on duty whenever the CAH has one or more inpatients. Federally, a licensed practical nurse (LVN in Texas) may be that on-duty nurse. The registered-nurse obligation is the provide-and-supervise role above, not a second nurse on every shift.

**Texas adds a requirement the federal rule does not.** [26 TAC 505.41(o)(2)(D)](https://www.law.cornell.edu/regulations/texas/26-Tex-Admin-Code-SS-505-41) states that "an RN shall be on duty in each building of a licensed hospital that contains at least one nursing unit where patients are present." So a Texas hospital cannot rely on the federal LVN allowance alone to staff an overnight: clearing §485.631(a)(5) with an LVN still leaves the state requirement open. Our guide to [nurse staffing models](/blog/nurse-staffing-models) works through what that means shift by shift.
...
Fix: Credential expiration dates need to be tracked somewhere you actually check before publishing a schedule, and the nurse's shift eligibility has to change the day a credential lapses. Most scheduling tools leave this as a manual update. SimpleScheduleAI does not track credential expiry either, so this one stays with you or with whatever system already holds your certification records; what we schedule against is the charge approval and competency level you record for each nurse.

**Gap 3: No documented emergency-care availability**

An overnight shift is covered by a licensed practical nurse on duty, which §485.631 permits federally, though a Texas hospital also has to satisfy the state RN-in-each-building rule above. What is missing here is the record showing that the required emergency-care practitioner, a physician, physician assistant, nurse practitioner, or clinical nurse specialist, was on call and available within the window §485.618 requires. The surveyor asks for that on-call record and it does not exist.
