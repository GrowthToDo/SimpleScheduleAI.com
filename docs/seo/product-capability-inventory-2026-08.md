...
| **SHIPPED** | Every escalation step attempted is recorded on the callout record (`escalationStepsTaken` JSON: step, attempted, result, timestamp). **This is STEP-level only. It does NOT capture who was phoned, what they said, or any call content** — a live post claimed it did and the founder corrected it on 2026-09-02. Never describe this as a record of the calls a manager made. | `callout` table in `src/db/schema.ts` |
