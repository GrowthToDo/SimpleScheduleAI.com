...
| `triad` | AI builds your schedule, our scheduling team checks it, you approve. No IT. | The AI builds your schedule, 22 automated rule checks validate every draft, you approve. Runs in the browser, no installation, no IT department required. | index.astro hero subtitle |
...
| `pricing-short` | $1,000 to $1,500 a month, by roster size. One price per hospital, not per nurse. | $10 per user per month, or $100 per user per year. No setup fees. | index.astro CallToAction subtitle, pricing.astro hero badge |
...
| `team-reference` | checked by our scheduling team / our scheduling team checks it (any "our scheduling team" construction) | validated by the built-in quality checks (22 automated rules) | index.astro, how-it-works.astro, pricing.astro |
...
| `system-maintains` | our scheduling team maintains the [per-diem bench / availability list / roster] and keeps it current | the software maintains the [artifact] and keeps it current | NEVER attribute roster/list maintenance to '22 automated rule checks' (those validate drafts only); generic software subject is the honest product form (round-2 Major finding) |
...
| `value-prop` | Every shift covered, without leaning on the same few nurses. Nurse scheduling for Texas hospitals of 25 beds or fewer. | Every shift covered, without leaning on the same few nurses. Nurse scheduling software for hospitals of 25 beds or fewer. | index.astro meta description, product.astro meta description |
| `promise-coverage` (added 2026-08-21) | Every shift covered, without leaning on the same few nurses. | Every shift covered, without leaning on the same few nurses. | index.astro hero subtitle, product.astro hero subtitle |
| `promise-predictability` (added 2026-08-21) | A schedule your nurses can build a life around, cycle after cycle. | A schedule your nurses can build a life around, cycle after cycle. | OutcomeStrip card 04 |
| `hours-returned` (added 2026-08-21) | Most of those hours go back to the floor. What's left is 1 to 2 hours of review. | Most of those hours go back to the floor. What's left is 1 to 2 hours of review. | index.astro hero content |
| `accountability` (added 2026-08-21) **SERVICE-ONLY** | When something slips on our side, you have a name and a number, not a ticket queue. | *No product twin exists, by design. In product mode the hospital runs the software and there is no "our side" accountable for a schedule we did not build. Writing a twin would manufacture a support promise the product tier does not make.* | pricing.astro hero subtitle |
| `rotation-vs-agency` (added 2026-08-21) | An agency booking covers tonight. It doesn't change who gets asked tomorrow. | An agency booking covers tonight. It doesn't change who gets asked tomorrow. | index.astro FAQ |
| `excel-counter` (added 2026-08-21) | A spreadsheet does what you tell it. It won't tell you the shift has nobody charge-qualified on it. | A spreadsheet does what you tell it. It won't tell you the shift has nobody charge-qualified on it. | SoftwareVsService intro |
...
| 2026-08-21 | Adopted 8 rows from `docs/strategy/POSITIONING.md` after founder approval. AMENDED `value-prop` (was a deliverable pitch aimed at the administrator's budget scoreboard) and `pricing-short` (`No IT setup` / `No per-nurse fees` demoted out of the headline slot under More-for-More; replaced by `One price per hospital, not per nurse`, which keeps the anti-per-seat signal without the affordability identity). ADDED `promise-coverage`, `promise-predictability`, `hours-returned`, `accountability` (SERVICE-ONLY, no product twin is possible by design), `rotation-vs-agency`, `excel-counter`. Shipped to index.astro, pricing.astro, product.astro, SoftwareVsService.astro, OutcomeStrip.astro the same day. | Claude |
