...
| GA4 AI-referral tracking | **DO — via MCP access** (both founders have GA4 access; property G-J2WK4QR3RC) | WIRED 2026-08-03: official analytics-mcp==0.7.0 (pinned, Bing-breakage lesson) in .mcp.json (gitignored, verified), service account ssai-analytics-mcp@simplescheduleai-seo.iam.gserviceaccount.com granted Viewer on property 528589584 by the founder. Clean launch verified. NEXT: founder reconnects the MCP, then run the first AI-referral query (sessionSource matching chatgpt/perplexity/claude/copilot/gemini) and add the standing row to the monthly audit runbook — no GA4-UI channel-group config strictly required. Optional cosmetic: create the "AI Search" channel group in the UI later for founder dashboards. |
