...
| GA4 AI-referral tracking | **DO — via MCP access** (both founders have GA4 access; property G-J2WK4QR3RC) | IN SETUP: add the official Google Analytics MCP to `.mcp.json` (gitignored). Once wired, the monthly audit queries AI-referral sessions (sessionSource matching chatgpt/perplexity/claude/copilot/gemini) via the Data API directly — no GA4-UI channel-group config strictly required. Optional cosmetic: create the "AI Search" channel group in the UI later for founder dashboards. |
