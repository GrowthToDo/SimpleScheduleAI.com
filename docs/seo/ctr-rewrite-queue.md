...
**Q1 follow-up, parked until the 2026-10-01 readout (decided 2026-09-26):** the generic hospital-shift queries ("hospital shift schedule" 25, "typical nursing shifts" 20, "hospital shifts" 18, "how do nurses schedules work" 15, "hospital shifts for nurses" 13, "typical nurse schedule" 12, plus smaller variants; 90-day pull) ALL land on the 12-hour post. Do NOT publish the `what-is-hospital-shift-scheduling` draft as a separate page (it would cannibalize these queries, and it carries a Pitman error: "consistent 42-hour weeks" is wrong, Pitman alternates 60 and 24). Instead, after the Q1 title test is measured, add one short H2 to the 12-hour post on typical hospital shift times for 8- and 12-hour patterns (use the free template's own Shifts tab times, labelled as the template's, not as an industry norm) and link the 8-vs-12 post. Waiting keeps the title test unconfounded.
